...
def evaluate_file(path: str):
    ...
    for rec in data:
        ...
        if language not in llm_em_list:
            llm_em_list[language] = []
        if language not in llm_f1_list:
            llm_f1_list[language] = []
        llm_em_list[language].append(llm_em)
        llm_f1_list[language].append(llm_f1)

        mt2_em = max_over_answers(exact_match, mt2_ans, gts)
        mt2_f1 = max_over_answers(f1_score, mt2_ans, gts)
        if language not in mt2_em_list:
            mt2_em_list[language] = []
        if language not in mt2_f1_list:
            mt2_f1_list[language] = []
        mt2_em_list[language].append(mt2_em)
        mt2_f1_list[language].append(mt2_f1)
    ...
    metrics = {
        "num_samples": len(data),
        "llm_exact_match": {language: pct(mean(llm_em_list[language])) for language in llm_em_list},
        "llm_f1": {language: pct(mean(llm_f1_list[language])) for language in llm_f1_list},
        "mt2_exact_match": {language: pct(mean(mt2_em_list[language])) for language in mt2_em_list},
        "mt2_f1": {language: pct(mean(mt2_f1_list[language])) for language in mt2_f1_list},
    }
    print(json.dumps(metrics, ensure_ascii=False, indent=2))

def evaluate_file_end_to_end(path: str):
    with open(path, "r", encoding="utf-8") as f:
        data = json.load(f)
    ...
    for rec in data:
        ...
        mt2_ans = (rec.get("result") or {}).get("answer", "") or ""
...
def main():
    ...
    parser.add_argument(
        "--input",
        type=str,
        default="./results/end_to_end_piqa.json",
        help="path to output_error_cascade_mkqa.json",
    )
    args = parser.parse_args()
    if "end_to_end" in args.input:
        evaluate_file_end_to_end(args.input)
    else:
