...
def entity_swap_error(text: str, rng: random.Random) -> Optional[str]:
    ...
    if len(nums) >= 2:
        ...
        return out
    # fallback: small word-order jitter (swap adjacent tokens)
...
def omission_error(text: str, rng: random.Random, max_words: int = 2) -> Optional[str]:
    ...
    # remove a prepositional phrase subtree
    ...
    if preps:
        ...
        if len([tok for tok in span]) <= max_words:
            ...
            return out.strip()
    # fallback: remove the first N non-punct tokens
    if max_words > 0:
        tokens = [t for t in doc if not t.is_punct and not t.is_space]
        if tokens:
            # try removing 2 tokens first, then 1
            for n in (2, 1):
                if n > max_words:
                    continue
                if len(tokens) < n:
